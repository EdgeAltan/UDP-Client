...
void udp_receive_callback(void *arg, struct udp_pcb *upcb, struct pbuf *p, const ip_addr_t *addr, u16_t port)
{
	//Copy the data from the packet buffer (pbuf)
	strncpy (buffer, (char *)p->payload, p->len);

	counter++; //(message count increase)
...
void udpClient_Connect(void)
{
	...
	//First create a new UDP control block
	upcb = udp_new();
	...
	//Configure destination (server) IP address and port
	...
	if (err == ERR_OK)
	{
		//Secondly send message to server
		udpClient_send ();
